...
public interface PostActionRepository extends JpaRepository<PostAction, Long>,
    ...
    @Query(value = " select pa.post_action_id as postActionId, 0 as postType, null as userWhoShared, pa.created_at as createdAt " +
            ...
            " left outer join qr_post_actions pa on " +
            " pa.community_id=qrCommunity.community_id " +
            " where qrCommunity.community_id=:communityId and pa.deleted=false " +
            " and pa.created_at>=:lastPosted group by pa.post_action_id " +
            " order by 4 "
            , nativeQuery = true)
    List<Object[]> findAllByCommunityPostsByLastPosted(@Param("communityId") Long communityId,
                                                       @Param("lastPosted") Date lastPosted);
...
//            " where user=:user order by pa.createdAt desc ")
    ...
    @Query(value = " select pa.post_action_id as postActionId, 0 as postType, null as userWhoShared, pa.created_at as createdAt " +
            ...
            " inner join qr_shared_post_actions spa " +
            " on spa.user_actor_id=uf.user_id " +
            " where qrUser.id=:userId and spa.created_at>=:lastPosted " +
            " order by 4"
            , nativeQuery = true)
    List<Object[]> getUserFeedByLastPost(@Param("userId") Long userId, @Param("lastPosted") Date lastPosted);
